    ...
    <div className="mapa-container fade-in">
      <div className="mapa-header-card">
        <div className="header-titles">
          <h2>Mapa del Campus - UTN (Admin)</h2>
          <p>Supervisa el estado y accede al historial de recolecciones de cada contenedor.</p>
        </div>
        <div className="mapa-leyenda-moderna">
          <div className="leyenda-pill verde"><span className="dot dot-green"></span><span>Óptimo (&lt; 50%)</span></div>
          <div className="leyenda-pill amarillo"><span className="dot dot-yellow"></span><span>Atención (50% - 80%)</span></div>
          <div className="leyenda-pill rojo"><span className="dot dot-red"></span><span>Crítico (&gt; 80%)</span></div>
        </div>
      </div>
      ...
      <div className="mapa-layout-split">
        <div className={`mapa-seccion ${facultadSeleccionada ? 'mapa-reducido' : 'mapa-completo'}`}>
          {loading ? (
            ...
            <MapContainer center={UTN_CENTER} zoom={19} scrollWheelZoom={true} className="leaflet-map-container" style={{ height: '600px', width: '100%', borderRadius: '12px', position: 'relative' }}>
              ...
              {facultadesAgrupadas.map(fac => (
                <Marker key={fac.id} position={[fac.latitud, fac.longitud]} icon={crearIconoFacultad(fac.nivelPromedio, fac.codigo)}
                  eventHandlers={{ click: () => { setFacultadSeleccionada(fac); }, }}>
                    ...
                    <div className="mini-card-footer" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
                      ...
                      <button
                        onClick={() => {
                          ...
                          localStorage.setItem('ultimoContenedorId', id);
                          navigate(`/admin/colecciones?contenedorId=${id}`);
                        }}
                        style={{
...
export default MapaCampusAdmin;